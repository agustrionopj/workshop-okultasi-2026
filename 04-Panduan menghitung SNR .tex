...
Model ini mengasumsikan kondisi pengamatan relatif stabil dan belum secara eksplisit memasukkan semua sumber noise sistematik (misalnya scintillation kuat atau residu kalibrasi), sehingga hasilnya perlu diperlakukan hanya sebagai estimasi.

Panduan berikut menjelaskan cara menghitung SNR menggunakan \textit{software} AstroImageJ (AIJ) dengan data yang diambil menggunakan \textsc{SharpCap}. Pastikan Anda sudah memiliki data yang siap dianalisis, lalu ikuti langkah--langkah berikut untuk mendapatkan nilai SNR dari data Anda.

\begin{enumerate}
	\item Unduh AIJ dari tautan berikut (\url{https://astroimagej.com/downloads/}). Sesuaikan dengan sistem operasi yang digunakan.
	\item Lakukan instalasi seperti biasa, semua pilihan dalam keadaan \textit{default}.
	\item Siapkan folder kerja yang berisi file--file yang akan dihitung nilai SNR-nya. Pada contoh ini hanya digunakan 5 file tanpa file kalibrator (bias, dark, flat). Pastikan nama file memiliki pola tertentu, misalnya \textbf{namatarget\_exptime}. Secara default, \textsc{SharpCap} akan memberi nama file secara berurutan.
	\begin{center}
	...
	\item Pilih \textit{File > Import > Image Sequence}, lalu akan muncul tampilan berikut. Pastikan opsi `Use virtual stack' dicentang.
	...
	\item Tekan tombol Alt sambil klik kiri pada bintang target, maka akan muncul profil seeing bintang tersebut. Catat nilai FWHM yang tertera pada judul gambar (pada contoh ini, FWHM = 6.77). Tekan tombol `Save Aperture' lalu tutup jendela tersebut. Jika ada kesalahan saat menandai bintang, klik tombol \includegraphics[height=1em]{aij_clear} untuk menghapus aperture dan mengulangi prosesnya. FWHM (\textit{Full Width at Half Maximum}) adalah ukuran yang digunakan untuk menggambarkan lebar profil cahaya bintang pada citra. Nilai FWHM ini akan digunakan untuk menentukan ukuran aperture dalam fotometri bukaan.
	\begin{center}
		\includegraphics[width=0.6\linewidth]{snr_aij_seeing}
	\end{center}
	
	\item Klik tombol \includegraphics[height=1em]{aij_set}, lalu ubah beberapa parameter berikut.
	\begin{center}
	...
	\begin{enumerate}
		...
		\item Untuk kotak (2):
		\begin{itemize}
			\item \textbf{CCD gain}: Nilai gain adalah faktor konversi antara jumlah elektron yang diterima sensor dan nilai digital yang tersimpan pada citra (ADU, \textit{Analog-to-Digital Unit}).
			Periksa \textbf{CCD gain} untuk kamera Anda, biasanya tercantum pada spesifikasi kamera (terlampir beberapa contoh kurva gain; sesuaikan dengan gain pada \textsc{SharpCap}). Jika data berasal dari ADC 12-bit yang diskalakan ke 16-bit, maka:
			\begin{equation}
			...
			\end{equation}
			Jika kamera Anda memiliki ADC 16-bit, nilai gain yang dimasukkan adalah nilai gain pada kurva.
			Pastikan terlebih dahulu bagaimana perangkat lunak akuisisi menyimpan skala ADU pada file (native atau hasil penskalaan), karena faktor 16 hanya berlaku untuk konversi skala 12-bit $\leftrightarrow$ 16-bit.
			...
			\item \textbf{CCD dark current}: adalah noise yang dihasilkan oleh sensor kamera meskipun tidak ada cahaya yang masuk. Dark current disebabkan oleh elektron yang dihasilkan secara termal dalam sensor, dan nilainya biasanya meningkat seiring kenaikan suhu. Pada pengamatan okultasi, di mana waktu bukaan sangat pendek, nilai dark current biasanya sangat kecil dibandingkan sinyal dari bintang target, terutama jika kamera dilengkapi pendingin. Oleh karena itu, untuk perhitungan SNR awal, Anda dapat mengabaikan dark current.
		\end{itemize}
	\end{enumerate}
	...
	\item Arahkan kursor ke bintang target lalu klik 1x, maka aperture berwarna hijau akan muncul. Klik juga bintang--bintang lain yang cukup terang, maka aperture merah akan muncul. Setelah selesai, tekan `Enter'.
	\begin{center}
		\includegraphics[width=0.6\linewidth]{snr_aij_select}
	\end{center}
	
	\item Setelah \textit{alignment} selesai, tekan OK/Yes lalu tutup jendela citra. Direktori baru bernama `aligned' akan muncul dan berisi citra hasil \textit{alignment}. 
    
	\underline{Catatan:} Jika proses tersebut tidak menghasilkan direktori `aligned', ulangi langkah \textit{Import Image Sequence} dan pastikan `Use virtual stack' dicentang.
	
	\item Lakukan \textit{File > Import > Image Sequence} sekali lagi, tetapi arahkan ke direktori `aligned' dan tekan OK. Jendela file citra akan muncul kembali. Periksa dengan tombol panah kanan dan kiri untuk memastikan semua \textit{frame} telah \textit{aligned}.
	...
	\end{center}
	\begin{enumerate}
		\item Klik 1x pada bintang target (pastikan bintang yang dipilih tidak salah. Jika salah, hapus dengan tombol \includegraphics[height=1em]{aij_clear}). Klik juga pada bintang lain sebagai pembanding (opsional, berguna jika Anda ingin melakukan fotometri diferensial). Sebagai contoh, dokumen ini menggunakan 1 bintang pembanding (aperture merah). Tekan `Enter' untuk melakukan fotometri bukaan secara otomatis untuk semua frame.
		\begin{center}
			\includegraphics[width=0.7\linewidth]{snr_aij_aphot}
		\end{center}
		
		\item Akan muncul banyak jendela; abaikan untuk sementara. Saat proses selesai, cari jendela `Measurements' (lihat gambar berikut) dan cari nilai SNR untuk target Anda (kolom Source\_SNR\_T1). Jika ingin mengetahui SNR untuk bintang pembanding, pada kasus ini Anda dapat melihat kolom Source\_SNR\_C2. Untuk ringkasan hasil, gunakan nilai median (atau \textit{sigma-clipped mean}) agar lebih tahan terhadap \textit{outlier}.
		\begin{center}
			\includegraphics[width=0.7\linewidth]{snr_aij_snr}
		\end{center}
		\item Lakukan validasi ringkas hasil SNR: periksa sebaran SNR antar-\textit{frame}, identifikasi \textit{outlier} yang jelas tidak fisis (misalnya akibat awan lewat atau pergeseran bintang), lalu putuskan apakah \textit{frame} tersebut dikeluarkan dari ringkasan akhir.
	\end{enumerate}
	\item Interpretasi cepat hasil: SNR $< 5$ umumnya menunjukkan deteksi lemah, SNR $> 10$ biasanya memadai untuk analisis awal, dan SNR $> 100$ umumnya baik. Gunakan rentang ini sebagai panduan praktis, bukan batas kaku.
	\item Selamat! Anda telah berhasil menghitung SNR pada data Anda.
\end{enumerate}
\end{document}